...
# category -> (action, delay_minutes, target_channel)
DECISION_MAP: dict[FailureCategory, tuple[RetryAction, int | None, str | None]] = {
    ...
    FailureCategory.CARD_EXPIRED: (RetryAction.SWITCH_CHANNEL, None, "upi"),
    FailureCategory.UNKNOWN: (RetryAction.NO_RETRY, None, None),
}

# Known bank maintenance window — don't schedule retries here
...
def decide(category: FailureCategory) -> dict:
    ...
    scheduled_for = None
    if action in (RetryAction.RETRY_IMMEDIATE, RetryAction.RETRY_SCHEDULED):
